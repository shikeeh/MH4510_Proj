...
mod_rf <- train(Avg.Salary.K.~., data = D_clean, method = "ranger",
...
mod_rf_tune <- train(Avg.Salary.K.~., data = D_clean, method = "ranger",
                     num.trees = 50, importance = 'impurity',
...
```{r}
#selecting variables for random forest (shouldnt exist if data cleaning removed it)
D_clean <- D_clean %>%
  select(-c(index, Job.Title, Salary.Estimate, Job.Description, Lower.Salary, Upper.Salary, cleaned_text, job_simp, desc_len))
```
...
X <- D_clean %>% 
  select(-c(index, Avg.Salary.K.))
y <- D_clean$Avg.Salary.K.
