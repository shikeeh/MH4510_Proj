...
Next we create a document-term matrix (DTM) for our job descriptions,
setting minimum word frequency to 40.
...
trees. We first train the data to obtain its optimal hyperparameters. We
then perform grid search for the optimal hyper-parameter values to
...
```{r, echo = FALSE, comment = "", warning = FALSE, fig.cap="\\label{fig:figs}Actual average salary vs. predicted average salary"}
#plot graph to compare actual vs. predicted
ggplot(result, aes(x = Y_salary, y = predictions)) +
  geom_point(colour = "darkgreen", alpha = 0.65) +
  geom_abline(intercept = 0, slope = 1, alpha = 0.5) +
  expand_limits(x = 25, y = 25) + 
  labs(x = "Actual Average Salary", y = "Predicted Average Salary")
```
...
cat("MAE:", round(caret::MAE(test_data$Y_salary, predictions_rf), 2), "\n")
...
cat("MAE:", round(caret::MAE(test_data$Y_salary, pred_y_xgb), 2), "\n")
...
only 9 features have importance values greater than 0, whereas the
...
plot_GCV <- vip(mars_cv, num_features = 20, geom = "col", type = "gcv", 
                aesthetics = list(fill = "darkgreen", alpha = 0.7)) + ggtitle("GCV")
plot_RSS <- vip(mars_cv, num_features = 20, geom = "col", type = "rss", 
...
#measure prediction accuracy
cat("MAE:", round(caret::MAE(test_data$Y_salary, pred_mars), 2), "\n")
...
#measure prediction accuracy
cat("MAE:", round(caret::MAE(test_pls$Y_salary, pred_pls), 2), "\n")
...
ggplot(coefficients_pls_df, aes(x=coefficients_pls, 
                      y=reorder(rownames(coefficients_pls_df), coefficients_pls))) +
  geom_bar(stat = "identity", fill="darkgreen", alpha = 0.65) +
  xlab("Coefficients") + ylab("Key words")
```
