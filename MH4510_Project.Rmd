...
```{r}
D_cleaned <- D_clean %>%
  select(Job.Title, Rating, Location, Headquarters, Size, Type.of.ownership, Industry, Sector, 
         Revenue, Competitors, Hourly, Employer.provided, Avg.Salary.K.,Job.Location, Age, 
         Python, spark, aws, excel, sql, sas, keras, pytorch, scikit, tensor, hadoop, tableau, 
         bi, flink, mongo, google_an, Degree, seniority)
```

```{r}
X <- D_cleaned %>% 
  select(-c(Avg.Salary.K.))
y <- D_cleaned$Avg.Salary.K.
...
```{r}
mod_rf <- train(Avg.Salary.K.~., data = D_cleaned, method = "ranger",
                num.trees = 50, importance = 'impurity',
                trControl = trainControl("oob"))
mod_rf

rfGrid <- expand.grid(mtry = 955,
                      min.node.size = 5,
                      splitrule = "extratrees")

mod_rf_tune <- train(Avg.Salary.K.~., data = D_cleaned, method = "ranger",
                     num.trees = 50, importance = 'impurity',
...
mod_rf_tuned <- train(Avg.Salary.K.~., data = D_cleaned, method = "ranger",
...
print(paste0('RMSE: ' ,caret::postResample(predictions , Y_test)['RMSE'])) #33.41
