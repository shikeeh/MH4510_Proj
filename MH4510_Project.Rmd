...
$$y=\left\{\begin{array}{1} \beta_0 + \beta_1(1-x) & \textrm{if x < 1} \\ \beta_0 + \beta_1(x-1) & \textrm{if x > 1} \end{array}\right$$
...
$$y=\left\{\begin{array}{2} \beta_0 + \beta_1(1-x) & \textrm{if x < 1} \\ \beta_0 + \beta_1(x-1) & \textrm{if x > 1 & x < 2} \\ \beta_0 + \beta_1(2-x) & \textrm{if x > 2} \end{array}\right$$
...
After pruning, a predictor's significance value is 0 if it was never used in any of the MARS basis functions in the final model. There are only 17 features have importance values greater than 0, whereas the other features all have importance values of zero since they were excluded from the final model. 
